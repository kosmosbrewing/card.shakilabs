...
// WHY THIS EXISTS
// ---------------------------------------------------------------------------
// This body is rendered twice from one source: as the static prerender, and by
// SeoRichContent through v-html (src/seo/routeRichContent.ts). Its tables carry
// width:100%, but a table cannot shrink below its min-content width - at 390px a
// currency table settles around 405px and, with nothing scrolling it, it sizes
// the DOCUMENT instead, so the reader drags the whole page sideways to read a
// paragraph. /overseas-payment did exactly that.
//
// The fix is the one the app already uses for its Vue tables
// (src/components/overseas/OverseasCompareTable.vue): a wrapper that scrolls,
// with tabindex="0" so the scroll box is reachable by keyboard, role="region"
// and a name so a screen reader announces what is being scrolled. The data is
// never narrowed - a currency column that wraps is worse than one that scrolls.
//
// Inline styles rather than the Tailwind class the Vue components use: Tailwind
// scans ./index.html and ./src only (tailwind.config content), so a utility whose
// only remaining reference lived in scripts/ would silently stop being generated
// and the wrapper would quietly stop scrolling - the exact failure mode the
// dead-utility sweep found across the fleet. The rest of the prerendered body is
// inline-styled for the same reason.
