...
// 히어로 수치가 만들어낼 수 있는 문자. 값은 전부 toLocaleString/포맷터 출력이고
// 증감 화살표까지 포함한다($=면세/해외결제, ▲▼=순혜택 부호). 카운트업 중간 프레임과
// 입력에 따라 바뀌는 자릿수(`,`가 생겼다 없어진다)는 렌더 한 장에 다 나오지 않아 따로 합친다.
// U+2212(−)는 이 폰트에 글리프가 없어 폴백(Pretendard)이 글리프 단위로 받는다.
export const NUMERAL_CHARACTERS =
  "0123456789,.%+-~/()\u00B7 $\u25B2\u25BC원억만천조년월일개회건세명점배급시간분초약";

// 브랜드 폰트 문자셋 = 브라우저 렌더 결과에서 GmarketSans로 그려지는 텍스트 ∪ 위 숫자셋.
// 소스 grep은 쓰지 않는다(주석·속성까지 세어 과대 수집된다 — docs/BRAND_FONT_SUBSET.md §3).
// 이 파일은 scripts/collect-brand-charset.mjs가 만들고, 같은 스크립트의 --check가
// 렌더 결과와 대조한다(npm run verify:brand-font, CI에서 크로미움 설치 후 실행).
const brandCharacters = JSON.parse(
  readFileSync(resolve(scriptRoot, "brand-charset.json"), "utf8")
).characters;

// 제목(font-title/font-brand)과 히어로 수치에 쓰는 브랜드 폰트. 전체 UI 문자셋(≈900자)으로
// 자르면 112KB, 렌더 실측 문자셋이면 그 1/10이다.
