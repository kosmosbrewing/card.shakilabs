...
// 강조 숫자(ResultHero) 전용 문자셋. 값은 전부 toLocaleString/포맷터 출력이라
// 숫자·통화 단위·증감 화살표면 충분하다($=면세/해외결제, ▲▼=순혜택 부호).
...
export const fontJobs = [
  ...
  {
    // 강조 숫자 전용. 이 폰트는 ResultHero의 히어로 수치 한 곳(font-numeral)에서만 쓰인다.
    // 전체 UI 문자셋으로 자르면 112KB, 아래 45자면 15KB 미만이다.
