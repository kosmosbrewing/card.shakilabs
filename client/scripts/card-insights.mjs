...
function annualFeeInsights() {
  ...
  const findings = [
    ...
    `통신비 ${won(spending.telecom)}에 10%를 붙이는 카드가 ${telecomCards.length}장이고 모두 월 ${won(telecomBenefit)}으로 같습니다. 그래서 통신비 하나만 보고 카드를 고르면 어느 쪽을 골라도 결과가 같고, 차이는 나머지 카테고리에서만 납니다.`,
  ];
  ...
  return {
    ...
    findings,
    groups: [
      { title: "카드 선택만으로 연 수백만 원이 갈린다", count: 2 },
      { title: "한도가 두 번 걸리는 카드가 있다", count: 2 },
      { title: "실적 1원 차이가 혜택 전체를 지운다", count: 2 },
      { title: "지출 규모가 바뀌면 1위도 바뀐다", count: 2 },
    ],
    table: {
...
function overseasInsights() {
  ...
  // Inline the fee-waiver condition on the sentence that names the cheapest card,
  // instead of leaving it to surface only a few findings later (readability QA #3).
  const cheapestBenefit = cheapest.card.benefits[0];
  const cheapestConditionNote =
    cheapestBenefit && cheapestBenefit.minSpend > 0
      ? `전월실적 ${won(cheapestBenefit.minSpend)}을 채우고 연회비 ${won(cheapest.card.annualFee)}을 낸 `
      : "";
  ...
  const sameCapNote = sameCapPair
    ? ` 그런데 한도가 같은 ${won(sameCapPair.cap)}이라도 적립률이 ${pct(Math.min(sameCapPair.rate, sameCapOther.rate))}면 ${pct(Math.max(sameCapPair.rate, sameCapOther.rate))}보다 늦게 걸리므로, 한도 액수만으로 카드를 비교하면 순서가 틀립니다.`
    : "";
  const findings = [
    `수수료율은 ${feeGroups.size}단계뿐입니다. 신용카드 ${creditFee.length}장은 환전 ${pct(creditFee[0].card.exchangeFeeRate)}+브랜드 ${pct(creditFee[0].card.networkFeeRate)}로 전부 ${pct(creditFee[0].feeRate)}이고, 체크 ${zeroFee.length}장은 0%, 선불 1장은 ${pct(results.find((result) => result.cardId === "travel-wallet").feeRate)}입니다. 그래서 ${usd(1000)}(${won(cheapest.baseKrwAmount)}) 결제에서 신용카드끼리는 수수료가 ${won(creditFee[0].cardFeeAmount)}으로 한 푼도 다르지 않습니다.`,
    `수수료에서 혜택을 뺀 실부담으로 11장을 세우면 ${usd(1000)} 결제에서 ${cheapestConditionNote}${cardName(cheapest.card)} ${won(cheapest.localCurrencyNet)}이 가장 적고, ${cardName(priciest.card)} ${won(priciest.localCurrencyNet)}이 가장 많습니다. 그 결과 폭은 ${won(priciest.localCurrencyNet - cheapest.localCurrencyNet)}까지 벌어지는데, 결제액의 ${pct((priciest.localCurrencyNet - cheapest.localCurrencyNet) / cheapest.baseKrwAmount, 2)}에 해당합니다.`,
    `혜택률이 자기 수수료율보다 높은 카드는 11장 중 ${beatsFee.length}장(${beatsFee.map((result) => cardName(result.card)).join("·")})뿐입니다. 나머지 신용카드는 0.5~0.8% 적립을 받아도 1% 수수료를 넘지 못하기 때문에, 수수료 0원 체크카드보다 실부담이 항상 큽니다.`,
    ...
    `${cardName(toss.card)}의 1% 캐시백은 월 ${won(tossCap.monthlyCap)} 한도라 총액 ${won(tossCapAtTotal)}에서 멈춥니다. 그래서 ${usd(1000)} 결제면 이미 한도를 넘어 실제 환급률은 ${pct(tossEffective, 2)}로 내려갑니다.`,
    `적립 한도가 걸리는 월 결제액은 ${capSpends.map((row) => `${row.name} ${won(row.capAt)}`).join(", ")}입니다.${sameCapNote}`,
    `원화 결제(DCC)를 고르면 카드 수수료와 상관없이 환율에 ${pct(DCC_MARKUP.defaultRate, 0)}가 덧붙습니다. 그래서 ${usd(1000)}에서는 ${won(dccDefault)}으로, 가장 비싼 카드 수수료(${won(dccWorstFee)})의 ${times(dccDefault / dccWorstFee)}에 이릅니다. 마크업 범위 ${pct(DCC_MARKUP.typicalMinRate, 0)}~${pct(DCC_MARKUP.typicalMaxRate, 0)}를 대입하면 ${won(dccMin)}~${won(dccMax)}입니다.`,
    `전월 실적이 붙은 카드는 ${minSpendCards.length}장입니다. 그래서 ${usd(200)}(${won(smallTotal)})짜리 결제 한 건만 보면 ${smallWarnings}장 모두 실적 경고가 뜨고, 가장 높은 실적 ${won(highestMinSpend)}을 한 건으로 넘기려면 ${usd(clearsAllUsd)} 이상을 결제해야 합니다.`,
  ];
  ...
  return {
    ...
    findings,
    groups: [
      { title: "수수료율은 3단계뿐, 승부는 혜택률에서 갈린다", count: 3 },
      { title: "적립에는 상한이 있다", count: 3 },
      { title: "DCC와 전월 실적, 두 개의 함정", count: 2 },
    ],
    table: {
...
function fuelCardInsights() {
  ...
  const findings = [
    `리터당 할인은 유가에 따라 할인율이 달라집니다. ${FUEL_PRICES.lastUpdated} 휘발유 ${won(price)}/L 기준으로 ${equivalents.map((row) => `${row.amount}원/L는 ${pct(row.gasoline, 2)}`).join(", ")}에 해당합니다. 그런데 LPG(${won(FUEL_PRICES.lpg)}/L)에서는 같은 ${equivalents[0].amount}원/L가 ${pct(equivalents[0].lpg, 2)}, ${equivalents[equivalents.length - 1].amount}원/L가 ${pct(equivalents[equivalents.length - 1].lpg, 2)}로 뛰기 때문에, 정률 카드와의 우열이 유종에 따라 바뀝니다.`,
    ...
    `월 할인 한도가 처음 걸리는 주유액은 ${capAt.map((row) => `${row.name} ${won(row.capAt)}`).join(", ")}입니다. 그래서 한도 ${won(capAt[0].cap)}인 카드는 이 금액을 넘는 주유에 대해 추가 할인이 0원입니다.`,
    `주유비만으로 전월 실적을 채우는 카드는 ${won(200000)} 기준 ${clears200.length}장(${clears200.map((result) => result.card.name).join("·")})뿐이고, ${warnings200.length}장은 실적 경고가 뜹니다. 그런데 이 계산기는 주유액만 받기 때문에, 다른 지출로 실적을 채우는 경우는 실적 조건 계산기에서 확인해야 합니다.`,
    `연회비를 월로 나눈 금액을 할인으로 상쇄하는 최소 주유액은 ${feeBreakEven.map((row) => `${row.name} ${won(row.spend)}`).join(", ")}입니다. 그래서 이 금액 아래에서는 실적을 채워도 카드가 연회비만큼 손해입니다.`,
    `${hyundai.name}(10%)와 ${lotte.name}(${lotte.discount.amount}원/L)의 우열은 월 주유 ${won(crossover)}에서 한 번 뒤집힙니다. 그 아래에서는 10%가 앞서지만 ${hyundai.name}는 ${won(hyundaiCapAt)}부터 첫 구간 한도 ${won(fuelTiers(hyundai)[0].monthlyCap)}에 묶이는 반면, ${lotte.name}는 ${won(lotteCapAt)}까지 할인이 계속 늘어 ${won(300000)}에서 ${won(lotte300.monthlyNet)} 대 ${won(hyundai300.monthlyNet)}으로 벌어집니다.`,
    `같은 ${won(200000)}으로 ${litersByFuel.map((row) => `${row.label} ${row.liters}L`).join(", ")}를 넣습니다. 리터당 카드는 리터 수에 비례하기 때문에 ${lotte.name}의 할인이 휘발유 ${won(lotteGas200.monthlyDiscount)}에서 LPG ${won(lotteLpg200.monthlyDiscount)}으로 ${pct(lotteLpg200.monthlyDiscount / lotteGas200.monthlyDiscount - 1, 0)} 늘어납니다.`,
    `${soil.card.name}는 S-Oil 전용이라 선호 주유소를 다른 브랜드로 고르면 비교에서 빠집니다. 그런데 브랜드 제한이 없을 때는 ${won(300000)} 기준 순절약 ${won(soil.monthlyNet)}으로 6장 중 ${soilRank300}위이므로, 주유소를 가리지 않는 운전자에게만 의미가 있는 순위입니다.`,
  ];
  ...
  return {
    ...
    findings,
    groups: [
      { title: "유가·유종이 바뀌면 같은 할인율도 다르게 작동한다", count: 2 },
      { title: "주유액이 늘어나면 1위도 바뀐다", count: 2 },
      { title: "한도와 실적이 실제 혜택을 깎는다", count: 3 },
      { title: "브랜드 제한은 순위를 접고 봐야 한다", count: 1 },
    ],
    table: {
...
function minSpendInsights() {
  ...
  const findings = [
    `화면 기본값은 생활비 7개 항목 ${won(householdTotal)}에 주유 ${won(200000)}을 더한 월 ${won(total)}입니다. 이 금액은 6장 전부의 실적 문턱(${won(Math.min(...minSpends))}~${won(Math.max(...minSpends))})을 넘기 때문에, 기본 화면에서는 어떤 카드도 실적 경고가 뜨지 않습니다.`,
    `같은 주유 ${won(200000)}을 생활비 없이 넣으면 ${fuelOnlyUnmet.length}장이 실적 미달로 할인 0원이 됩니다. 그런데 생활비를 함께 입력하는 순간 6장 모두 충족으로 바뀝니다. 이 계산기가 주유비 외 7개 항목을 묻는 이유가 여기 있습니다.`,
    ...
    `${hyundai.name}의 두 번째 구간은 한도를 ${won(tier2Step)} 올려 주지만 세 번째 구간은 지출 ${won(hyundaiTiers[2].minSpend - hyundaiTiers[1].minSpend)}을 더 요구하며 한도를 ${won(tier3Step)} 올립니다. 추가 지출 대비 늘어나는 한도는 ${pct(tier3Marginal, 2)}뿐이라서, 세 번째 구간을 노리고 지출을 늘리는 것은 주유가 그만큼 많을 때만 의미가 있습니다.`,
    `생활비를 절반(${won(halvedTotal - 200000)})으로 줄이면 총지출 ${won(halvedTotal)}에서 ${halvedUnmet.length === 0 ? "미달로 바뀌는 카드는 없지만" : `${halvedUnmet.length}장이 미달로 바뀌고`} ${hyundai.name}의 한도는 ${won(hyundaiNow.appliedMonthlyCap)}에서 ${won(halvedHyundai.appliedMonthlyCap)} 구간으로 내려갑니다. 그런데 실적은 채웠는데도 할인이 줄어드는 경우가 있다는 뜻입니다.`,
    `실적을 채우려고 지출을 늘리면 그 금액도 비용입니다. ${lotte.name}는 주유 ${won(200000)}에서 할인 ${won(lotteRawDiscount)}에 연회비 월 ${won(lotte.annualFee / 12)}을 빼면 ${won(lotteGapCeiling)}이 남습니다. 그래서 부족한 실적이 ${won(lotteGapCeiling)}을 넘으면 억지로 채우는 편이 손해입니다. 화면의 '추가 지출까지 감안한 순혜택'이 이 계산입니다.`,
    `실적 문턱이 가장 낮은 카드(${cheapestEntry.map((card) => card.name).join("·")}, ${won(Math.min(...minSpends))})는 연회비도 ${won(Math.min(...cheapestEntry.map((card) => card.annualFee)))}으로 가장 싸고, 가장 높은 카드(${steepestEntry.map((card) => card.name).join("·")}, ${won(Math.max(...minSpends))})는 연회비 ${won(Math.max(...steepestEntry.map((card) => card.annualFee)))}으로 가장 비쌉니다. 그래서 문턱과 연회비가 같은 방향으로 움직이며, 진입이 쉬운 카드가 유지 비용도 낮습니다.`,
    `기본값에서 월 순혜택은 ${ranked[0].card.name} ${won(ranked[0].monthlyNetBenefit)}이 1위, ${ranked[ranked.length - 1].card.name} ${won(ranked[ranked.length - 1].monthlyNetBenefit)}이 마지막입니다. 다만 주유소 브랜드 제한이 있는 ${branded.map((card) => card.name).join("·")}는 선호 주유소를 지정하는 순간 이 순위에서 빠질 수 있습니다.`,
  ];
  ...
  return {
    ...
    groups: [
      { title: "생활비를 더해야 실적 문턱을 넘는다", count: 2 },
      { title: "현대카드 3단계 구간, 오를수록 한도도 오른다", count: 2 },
      { title: "지출을 줄이거나 억지로 늘리면 손해가 난다", count: 2 },
      { title: "문턱이 낮은 카드가 연회비도 싸다", count: 2 },
    ],
    findings,
...
function mileageInsights() {
  ...
  const findings = [
    ...
    `이코노미 공제 마일은 6개 노선 전부 두 항공사가 ${sameEconomy ? "같습니다" : "다릅니다"}. 차이는 장거리 비즈니스에서 나는데, 유럽 비즈니스가 대한항공 ${formatWon(keBiz.milesRequired)}마일 대 아시아나 ${formatWon(ozBiz.milesRequired)}마일로 아시아나가 ${pct(1 - ozBiz.milesRequired / keBiz.milesRequired, 0)} 적기 때문에, 같은 운임 기준 1마일 가치가 ${won(keBiz.valuePerMile)} 대 ${won(ozBiz.valuePerMile)}로 벌어집니다.`,
    `같은 70,000마일로 ${seventy.map((item) => `${item.routeLabel} ${won(item.cashPrice)}`).join(", ")} 이코노미를 탈 수 있습니다. 공제 마일이 같으니 1마일 가치는 ${won(seventy[0].valuePerMile)}에서 ${won(seventy[seventy.length - 1].valuePerMile)}으로 ${pct(seventy[seventy.length - 1].valuePerMile / seventy[0].valuePerMile - 1, 0)} 차이가 납니다. 즉 장거리 세 노선 중 어디를 고르느냐가 마일 가치를 정합니다.`,
    `대한항공 유럽은 비즈니스가 이코노미보다 마일을 ${times(keBiz.milesRequired / keEuropeEco.milesRequired, 2)} 요구하지만 운임은 ${times(keBiz.cashPrice / keEuropeEco.cashPrice, 2)}입니다. 상위 좌석이 유리한 이유가 이 두 배율의 차이입니다. 그래서 일등석은 ${formatWon(keFirstEurope.milesRequired)}마일에 ${won(keFirstEurope.cashPrice)}으로 ${won(keFirstEurope.valuePerMile)}, 아시아나는 ${formatWon(ozFirstEurope.milesRequired)}마일이라 ${won(ozFirstEurope.valuePerMile)}입니다.`,
    `보유 마일에 따라 열리는 공제 항목 수는 30,000마일에 ${eligibleAt(30000)}개, 45,000마일에 ${eligibleAt(45000)}개, 70,000마일에 ${eligibleAt(70000)}개, 125,000마일에 ${eligibleAt(125000)}개입니다. 그래서 70,000마일이 장거리 이코노미와 동남아 비즈니스가 동시에 열리는 첫 문턱입니다.`,
    `포인트 전환 계산기는 대한항공 1마일을 ${won(pointValueKe)}, 아시아나를 ${won(pointValueOz)}으로 잡습니다. 이 공제표 기준으로 그 가치 이상이 나오는 항목은 대한항공 18개 중 ${aboveKe}개, 아시아나 18개 중 ${aboveOz}개뿐입니다. 그래서 그 가정은 이코노미 평균보다 높고 비즈니스 평균보다 낮은 지점에 있습니다.`,
    `한일·한중 이코노미 4개 항목은 1마일이 최대 ${won(shortEcoMax)}에 그쳐 ${won(pointValueKe)} 가정의 ${pct(shortEcoMax / pointValueKe, 0)} 수준입니다. 그래서 단거리 이코노미에 마일을 쓰면 가정의 절반도 회수하지 못한다는 것이 표에서 바로 나오는 결론입니다.`,
  ];
  ...
  return {
    ...
    findings,
    groups: [
      { title: "마일 가치는 노선·좌석에 따라 널뛴다", count: 2 },
      { title: "같은 이코노미인데 항공사마다 다르다", count: 2 },
      { title: "상위 좌석일수록, 보유 마일이 많을수록 유리해진다", count: 2 },
      { title: "환산 가정과 실제 공제표는 다른 자리에 있다", count: 2 },
    ],
...
function dutyFreeInsights() {
  ...
  const findings = [
    `간이세율은 이 계산기에서 한 번도 선택되지 않습니다. 관세 8%에 부가세를 얹은 실효세율 ${pct(stackedRate(0.08, vatRate))}가 간이세율 ${pct(cosmeticsSimplified.simplifiedRate, 0)}보다 낮고, 13% 품목도 ${pct(stackedRate(0.13, vatRate))} 대 ${pct(clothingSimplified.simplifiedRate, 0)}로 정식 계산이 항상 낮습니다. 그래서 간이세율이 붙은 ${withSimplified.length}개 품목 중 간이세율이 이기는 경우는 ${simplifiedWins.length}건입니다.`,
    ...
    `면세 한도 ${usd(exemptionLimitUsd)}는 초과분에만 과세하는 구조라 절벽이 없습니다. ${usd(exemptionLimitUsd + 1)}를 사면 화장품 기준 세금은 ${won(justOver.finalTax)}입니다. 반면 해외직구 관세 계산기의 ${usd(CARD_CUSTOMS_CATEGORIES[0].thresholdUsd)} 기준은 총액 전체에 과세하는 전부-아니면-전무 방식이라서, 두 계산기의 한도는 성격이 다릅니다.`,
    `${usd(1000)}짜리 화장품은 과세 대상이 ${usd(cosmetics1000.taxableAmountUsd)}(${won(cosmetics1000.taxableAmountKrw)})이고 세금은 ${won(cosmetics1000.finalTax)}, 구매액 전체 대비 ${pct(cosmetics1000.effectiveTaxRate, 2)}입니다. 그런데 같은 금액의 주류는 ${won(alcohol1000.finalTax)}으로 화장품의 ${times(alcohol1000.finalTax / cosmetics1000.finalTax)}에 달합니다.`,
    ...
    `한도를 넘는 ${usd(100)}마다 붙는 세금은 8% 품목 ${won(perHundred(0.08))}, 13% 품목 ${won(perHundred(0.13))}, 주류 ${won(perHundred(0.3))}, 담배 ${won(perHundred(0.4))}입니다. 초과분이 같아도 세율 자체가 다르기 때문에 세금이 ${times(perHundred(0.4) / perHundred(0.08))} 차이 납니다.`,
    `${usd(1500)} 의류는 과세 대상 ${usd(clothing1500.taxableAmountUsd)}에 세금 ${won(clothing1500.finalTax)}, 총비용 ${won(clothing1500.totalCostKrw)}으로 구매액의 ${pct(clothing1500.effectiveTaxRate)}가 세금입니다. 그런데 간이세율 상한 ${usd(clothingSimplified.simplifiedMaxUsd)} 안쪽인데도 정식 계산이 선택됩니다.`,
    `이 계산기의 가정 환율은 1달러 ${won(exchangeRate)}이고 해외직구 관세 계산기는 ${won(CUSTOMS_MODEL.exchangeRate)}을 씁니다. ${won(customsGap)} 차이는 8% 품목의 ${usd(200)} 초과분에서 세금 ${won(cosmetics1000.finalTax)} 대 ${won(overageTaxAtCustomsRate)}, 즉 ${won(cosmetics1000.finalTax - overageTaxAtCustomsRate)} 차이로 나타납니다. 다만 둘 다 관세청 고시환율이 아닌 비교용 가정입니다.`,
  ];
  ...
  return {
    ...
    findings,
    groups: [
      { title: "간이세율은 이 계산기에서 이길 수 없다", count: 2 },
      { title: "한도는 절벽이 아니라 초과분에만 붙는다", count: 3 },
      { title: "품목이 다르면 세금도 몇 배씩 차이 난다", count: 2 },
      { title: "환율 가정은 계산기마다 다르다", count: 1 },
    ],
...
function creditVsDebitInsights() {
  ...
  const findings = [
    `화면 기본값(월 ${won(defaults.monthlySpend)}, 연회비 ${won(defaults.annualFee)}, 신용 ${pct(defaults.creditRate)} 대 체크 ${pct(defaults.debitRate)})에서 연 혜택은 신용 ${won(base.annualCreditBenefit)} 대 체크 ${won(base.annualDebitBenefit)}으로 신용이 ${won(base.gap)} 앞섭니다. 즉 손익분기 지출은 월 ${won(base.breakEvenSpend)}이고, 기본값은 분기점의 ${times(defaults.monthlySpend / base.breakEvenSpend)} 지점에 있습니다.`,
    `연회비를 연회비 회수 계산기의 카드 8장 값으로 바꾸면 손익분기 지출은 ${feeRows.map((row) => `${won(row.annualFee)}→월 ${won(row.breakEven)}`).join(", ")}입니다. 즉 연회비가 ${times(highestFee / fees[0])}가 되면 분기점도 정확히 ${times(highestFee / fees[0])}가 되는 선형 관계입니다.`,
    `분기점을 움직이는 다른 변수는 혜택률 차이입니다. 연회비 ${won(defaults.annualFee)}에서 차이가 ${gapRows.map((row) => `${pctPoint(row.rateGap)}면 월 ${won(row.breakEven)}`).join(", ")}입니다. 그래서 차이가 ${pctPoint(gapRows[0].rateGap)}로 좁아지면 분기점이 기본 지출 ${won(defaults.monthlySpend)}을 넘어서고, 그 순간부터는 체크카드가 이깁니다.`,
    `기본 지출 ${won(defaults.monthlySpend)}을 고정하면 승자가 바뀌는 혜택률 차이는 ${pctPoint(flipGap, 2)}입니다. 다시 말해 신용카드 혜택률이 체크카드보다 이만큼도 높지 않다면 연회비 ${won(defaults.annualFee)}을 회수하지 못합니다.`,
    `연회비를 연간 결제액 대비 비율로 바꾸면 ${won(defaults.annualFee)}은 ${pct(feeAsRate(defaults.annualFee), 2)}, ${won(highestFee)}은 ${pct(feeAsRate(highestFee), 2)}입니다. 그래서 신용카드 혜택률에서 이 비율을 빼야 체크카드 혜택률과 같은 자리에서 비교됩니다.`,
    `입력 하한인 월 ${won(100000)}에서는 신용 ${won(minInput.annualCreditBenefit)} 대 체크 ${won(minInput.annualDebitBenefit)}으로 체크카드가 이깁니다. 같은 혜택률 차이라도 연회비가 고정비이기 때문에 지출이 작을수록 신용카드가 불리해지는 구조이며, 그 경계가 월 ${won(base.breakEvenSpend)}입니다.`,
    `연회비를 0원으로 두면 신용 ${won(zeroFee.annualCreditBenefit)} 대 체크 ${won(zeroFee.annualDebitBenefit)}으로 지출 규모와 무관하게 혜택률이 높은 쪽이 이기고 손익분기 지출은 계산되지 않습니다. 즉 연회비 면제 조건을 채우는 경우가 이 시나리오에 해당합니다.`,
    `연회비 회수 계산기의 카드 8장은 전월 실적이 최소 ${won(lowestMinSpend)}부터 시작합니다. 기본값의 손익분기 ${won(base.breakEvenSpend)}이 그보다 낮기 때문에, 실제로 신용카드를 막는 것은 손익분기가 아니라 실적 문턱입니다. 이 비교기는 실적과 소득공제율 차이를 반영하지 않습니다.`,
  ];

  return {
    heading: "손익분기 지출을 연회비·혜택률 전 구간에서 풀면",
    groups: [
      { title: "손익분기는 연회비에 선형으로 반응한다", count: 2 },
      { title: "혜택률 차이가 분기점을 더 크게 움직인다", count: 2 },
      { title: "지출 규모와 연회비 유무가 승자를 바꾼다", count: 3 },
      { title: "이 비교기가 반영하지 않는 것: 실적 문턱", count: 1 },
    ],
...
function pointConvertInsights() {
  ...
  const findings = [
    ...
    `${hotel.label}는 1단위에 ${won(hotel.unitValueWon)}이지만 ${hotel.pointsPerUnit}포인트가 1단위라 포인트당 실질 가치는 ${wonDec(hotelEffective)}입니다. 그런데 단위 가치만 보면 ${oz.label}(${won(oz.unitValueWon)})의 ${pct(hotel.unitValueWon / oz.unitValueWon, 0)}인데, 포인트당 실질가치로 보면 ${pct(hotelEffective / oz.unitValueWon, 0)}로 내려갑니다.`,
    `단위 환산은 내림이라 ${hotel.pointsPerUnit}로 나눠지지 않는 포인트는 버려집니다. 그래서 ${formatWon(1001)}포인트를 넣으면 ${oddLoss}포인트가 환산에서 빠지지만, 기본값 ${formatWon(defaults)}포인트는 ${formatWon(hotelRow.units)}단위로 딱 나눠져 손실이 없습니다.`,
    ...
    `${ke.label} ${won(ke.unitValueWon)} 가정은 마일리지 가치 계산기의 공제표와 대보면 이코노미 평균 ${won(keEco)}과 비즈니스 평균 ${won(keBiz)} 사이에 있습니다. 즉 어느 좌석에 쓸지 정해지지 않았다면 ${won(ke.unitValueWon)}은 낙관도 비관도 아닌 중간값입니다.`,
    `${ke.label}과 ${oz.label}의 가정 차이는 ${won(ke.unitValueWon - oz.unitValueWon)}(${pct(ke.unitValueWon / oz.unitValueWon - 1, 1)})이지만, 공제표에서는 유럽 비즈니스가 아시아나 ${won(europeBizOz.valuePerMile)} 대 대한항공 ${won(europeBizKe.valuePerMile)}으로 순서가 반대입니다. 그래서 장거리 비즈니스에 쓸 계획이면 이 페이지의 순위와 실제 회수 순위가 뒤집힙니다.`,
    `한일 이코노미처럼 1마일이 ${won(japanEco.valuePerMile)}에 그치는 사용처에 쓰면 ${formatWon(defaults)}마일의 실현 가치는 ${won(defaults * japanEco.valuePerMile)}으로 이 페이지 추정 ${won(top.estimatedValue)}의 ${pct(japanEco.valuePerMile / ke.unitValueWon, 0)}입니다. 그래서 추정치는 사용처가 정해질 때까지 상한에 가깝다고 봐야 합니다.`,
  ...
  return {
    ...
    groups: [
      { title: "환산 가정 네 줄 사이에도 격차가 크다", count: 2 },
      { title: "내림 규칙이 포인트를 깎는다", count: 2 },
      { title: "가정값과 실제 공제표는 다른 자리, 다른 순서에 있다", count: 3 },
      { title: "가정으로 연회비를 갚으려면 마일이 이만큼 필요하다", count: 1 },
    ],
...
function billingCycleInsights() {
  ...
  const findings = [
    `화면 기본값(구매 15일, 결제일 14일)은 우연이 아니라 최적 조합입니다. 결제일 다음 날 구매가 사용 주기 ${base.cycleDays}일에 유예 ${graceDays}일을 더해 ${base.usableDays}일로 가장 길고, 결제일 당일 구매는 주기 ${worst.cycleDays}일에 유예를 더한 ${worst.usableDays}일로 가장 짧습니다. 그래서 같은 카드, 같은 결제일에서도 구매일 하루 차이로 ${base.usableDays - worst.usableDays}일이 갈립니다.`,
    `매달 1일에 구매하는 사람의 무이자 사용 일수는 결제일 ${firstDayRows.map((row) => `${row.billingDay}일→${row.usable}일`).join(", ")}입니다. 그래서 결제일이 늦을수록 길어지며, 1일과 27일의 차이가 ${firstDayRows[firstDayRows.length - 1].usable - firstDayRows[0].usable}일입니다.`,
    `25일 월급을 받은 직후 구매한다면 결제일 ${paydayBest.billingDay}일이 ${paydayBest.usable}일로 가장 길고 결제일 ${paydayWorst.billingDay}일이 ${paydayWorst.usable}일로 가장 짧습니다. 그런데 월급일과 결제일을 맞추는 흔한 조언은 잔고 관리에는 맞지만, 무이자 기간으로는 오히려 최악의 선택입니다.`,
    `결제일을 어디에 두어도 31일 전체의 평균 사용 일수는 ${sameAverage ? `${averages[0]}일로 같습니다` : "다릅니다"}. 즉 결제일 변경은 평균을 늘리는 것이 아니라 어떤 날짜의 구매가 유리해지는지 재배치하는 일입니다.`,
    ...
    `이 계산기는 한 달을 ${monthDays}일, 청구 후 유예를 ${graceDays}일로 고정합니다. 30일 달에는 주기가 하루 짧아지고 유예 일수는 카드사마다 다르기 때문에, 화면의 일수는 상한에 가까운 추정치입니다.`,
    `결제일 14일 카드 한 장만 쓰면 구매 날짜에 따라 사용 일수가 최소 ${pairFloorSingle}일까지 내려갑니다. 결제일 1일 카드를 더해 매 구매에 유리한 쪽을 고르면 최소값이 ${pairFloor}일로 올라갑니다. 그래서 두 장의 결제일을 반달 간격으로 벌리는 것이 근거 있는 전략입니다.`,
    `최장 ${base.bestUsableDays}일은 결제일 선택지 ${BILLING_DAY_OPTIONS.length}개 어디에서나 결제일 다음 날 구매로 얻습니다. 즉 결제일 자체가 길이를 정하는 것이 아니라 구매 날짜와 결제일의 간격이 정한다는 것이 이 표의 결론입니다.`,
  ];
  ...
  return {
    ...
    findings,
    groups: [
      { title: "결제일보다 구매 타이밍이 기간을 정한다", count: 3 },
      { title: "결제일을 바꿔도 평균은 그대로, 재배치만 된다", count: 2 },
      { title: "이 계산기가 고정한 가정과 한계", count: 1 },
      { title: "카드를 겹치면 최소 보장 일수가 올라간다", count: 2 },
    ],
    table: {
...
function customsInsights() {
  ...
  const findings = [
    `화면 기본값(의류·신발 ${usd(defaults.productUsd)} + 배송비 ${usd(defaults.shippingUsd)})은 총액 ${usd(base.totalUsd)}로 한도를 넘어 관세 ${won(base.tariff)}에 부가세 ${won(base.vat)}이 붙고 세금 합계 ${won(base.totalTax)}, 최종 비용 ${won(base.landedCost)}입니다. 그래서 세금은 총액의 ${pct(base.totalTax / (base.totalUsd * exchangeRate))}에 해당합니다.`,
    ...
    `배송비도 총액에 들어갑니다. 전자기기 ${usd(140)}에 배송비 ${usd(20)}이면 총액 ${usd(shippingPush.totalUsd)}로 과세 대상이 되어 세금 ${won(shippingPush.totalTax)}이지만, 배송비가 ${usd(10)}이면 총액 ${usd(shippingSafe.totalUsd)}로 세금 ${won(shippingSafe.totalTax)}입니다. 그래서 배송비 ${usd(10)} 차이가 세금 ${won(shippingPush.totalTax - shippingSafe.totalTax)}으로 바뀝니다.`,
    ...
    `같은 ${usd(300)} 의류를 여행자가 들고 들어오면 면세 한도 계산기 기준 ${usd(DUTY_FREE_CONSTANTS.exemptionLimitUsd)} 아래라 세금 ${won(travelerSame.finalTax)}이고, 직구 소포로 받으면 이 계산기 기준 ${won(customs({ productUsd: 300, shippingUsd: 0, categoryKey: "fashion" }).totalTax)}입니다. 즉 한도가 ${usd(DUTY_FREE_CONSTANTS.exemptionLimitUsd)} 대 ${usd(threshold)}, 방식이 초과분 과세 대 총액 과세로 두 번 갈립니다.`,
    `골프용품 ${usd(300)}는 세금 ${won(golf300.totalTax)}을 더해 최종 ${won(golf300.landedCost)}, 물품가의 ${times(golf300.landedCost / (300 * exchangeRate), 2)}입니다. 그래서 국내 판매가와 비교할 때는 물품가가 아니라 이 배율을 곱한 값을 놓아야 합니다.`,
    `이 계산기의 환율은 1달러 ${won(exchangeRate)}으로 고정이라 ${usd(threshold)} 한도는 원화로 ${won(thresholdKrw)}입니다. 다만 면세 한도 계산기의 가정 환율 ${won(DUTY_FREE_CONSTANTS.exchangeRate)}과는 ${won(rateGap)} 차이가 있으며, 둘 다 관세청 고시환율이 아닙니다.`,
    `입력 상한(물품 ${usd(20000)} + 배송 ${usd(3000)})을 골프용품으로 넣으면 세금 ${won(maxInput.totalTax)}, 최종 ${won(maxInput.landedCost)}입니다. 이 계산기는 세 품목 모두 ${usd(threshold)} 한도를 쓰고 미국발 ${usd(200)} 특례나 목록통관 여부는 반영하지 않기 때문에, 미국 직구는 실제 세금이 화면보다 적을 수 있습니다.`,
  ];
  ...
  return {
    ...
    findings,
    groups: [
      { title: "150달러 한도는 절벽이다", count: 3 },
      { title: "실효세율은 관세율보다 높게 뛴다", count: 2 },
      { title: "같은 물건도 경로·계산기에 따라 세금이 다르다", count: 2 },
      { title: "이 계산기가 반영하지 않는 것", count: 1 },
    ],
...
// Findings used to render as one flat <ol> of eight-plus bullet paragraphs with
// no internal structure - readable to a data-recompute test, not to a person.
// `groups` (added per route above) clusters the same findings under a
// conclusion-style <h3> so a reader gets a claim to scan before the numbers
// that support it. The flat `findings` array stays the source of truth for the
// parity/length/similarity gates; grouping only changes how it is rendered.
function renderGroups(findings, groups) {
  const list = groups && groups.length > 0 ? groups : [{ title: null, count: findings.length }];
  let cursor = 0;
  return list
    .map((group) => {
      const slice = findings.slice(cursor, cursor + group.count);
      cursor += group.count;
      const items = slice.map((finding) => `<li style="${LI}">${finding}</li>`).join("");
      const heading = group.title ? `<h3 style="${H3}">${group.title}</h3>` : "";
      return `${heading}<ol style="${OL}">${items}</ol>`;
    })
    .join("");
}
...
export function buildInsightsSection(route) {
  ...
  return (
    ...
    renderGroups(insights.findings, insights.groups) +
